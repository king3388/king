...
class mobile extends base {
	...
	public function init(){	
		...
		$jinri_count = count($jinri_shoplist);
	...
	public function dataserver(){
		...
		//修改往期商品图片显示不了的问题2015-09-22
		$itempic = $this->db->GetOne("select picarr from `@#_shoplist` where `sid`='".$item['sid']."' order by id desc");
        $item['picarr'] = unserialize($itempic['picarr']);
		//记录	 
	...
	public function calResult(){
	  $itemid=intval($this->segment(4));
		...
		$user_shop_fmod = fmod($user_shop_time_add+$item['lottery_num'],$item['zongrenshu']);
