...
class pay {
	...
	//彩票开奖结果获取API
	public function kaicai(){
		$src = 'http://f.apiplus.cn/cqssc-10.json';
		$src .= '?_='.time();
		...
		for ($i = 0; $i < count($json->data); $i++) {
			...
			$a = $json ->data[$i]->opencode;
			//echo '<br>开奖号码：'.str_replace(",","",$a);
			//echo '<br>开奖时间：'.strtotime($json ->data[$i]->opentime).' | '.$json ->data[$i]->opentime;
			//echo '<br>';
			$datas[]=array('qishu'=>substr($p,0,8).'-'.substr($p,-3,3),'nums'=>str_replace(",","",$a),'time'=>strtotime($json ->data[$i]->opentime).' | '.$json ->data[$i]->opentime);
		}
		//获取最新一条记录
		$this -> datas = $datas[0];
		return $this -> datas;
	...
	private function addmoney_record($money=null,$data=null){		
		...
		if($pay_type['pay_class']=='weipay'){
			$_SESSION['wei_productid'] = $dingdancode;
			$_SESSION['title_name'] = _cfg('web_name');
			
			require_once "weipay/example/native.php";
			
			$out_trade_no = $_SESSION['wei_out_trade_no'];
			
			$query = $this->db->Query("INSERT INTO `@#_member_addmoney_record` (`uid`, `code`, `money`, `pay_type`, `status`,`time`,`score`,`scookies`,`out_trade_no`) VALUES ('$uid', '$dingdancode', '$money', '$pay_type[pay_name]','未付款', '$time','$score','$scookies','$out_trade_no')");
