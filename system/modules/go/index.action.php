...
class index extends base {
	...
	public function init(){		
		...
		//最新揭晓
		$shopqishu=$this->db->GetList("select id,sid,thumb,title,q_uid,q_user from `@#_shoplist` where `q_end_time` is not null and `q_showtime` = 'N' ORDER BY `q_end_time` DESC LIMIT 4");
		//云购动态
	...
	public function dataserver(){	
		...
		$lottery=$this->db->GetOne("select * from `@#_shoplist` where `id`='$itemid' and 'q_uid' is not null LIMIT 1");
